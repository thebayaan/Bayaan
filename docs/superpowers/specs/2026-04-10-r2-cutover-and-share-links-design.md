...
1. **Phase 1 — R2 Cutover:** Point all 243 rewayat server URLs in the Railway DB to `cdn.thebayaan.com`, handle backwards compatibility for cached data and stale player queue.
...
- 27,869 of 27,874 audio files are already on R2 bucket `bayaan-audio` at `cdn.thebayaan.com`
- 5 missing files are data quality issues (not real gaps)
- DB rewayat `server` column still points to old sources: 228 → mp3quran, 15 → Supabase, 14 → cdn.thebayaan.com
...
```
cdn.thebayaan.com/quran/recitations/{reciter-slug}/{rewayah-slug}/{style-slug}/default/{001-114}.mp3
```
...
After:  https://cdn.thebayaan.com/quran/recitations/abdulbasit-abdulsamad/hafs/murattal/default
```
...
5. Verifies `{r2_url}/001.mp3` returns HTTP 200 from `cdn.thebayaan.com` before writing
