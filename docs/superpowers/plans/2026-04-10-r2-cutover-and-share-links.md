...
**Goal:** Migrate all audio URLs to Cloudflare R2 (`cdn.thebayaan.com`), then build shareable deep links on `app.thebayaan.com` with auto-play, verse sharing, and rich OG previews.
...
for r in data['data']:
    for rw in r['rewayat']:
        ok = '✓' if rw['server'].startswith('https://cdn.thebayaan.com') else '✗'
        print(f'  {ok} {r[\"name\"]} / {rw[\"name\"]}: {rw[\"server\"][:60]}...')
"
```

Expected: All rows show `✓` with `cdn.thebayaan.com` URLs.
...
Verify audio plays correctly for each. Check the console logs for the audio URL — it should be a `cdn.thebayaan.com` URL.
...
```typescript
const APP_ICON_URL = 'https://cdn.thebayaan.com/assets/og-default.png';
```
